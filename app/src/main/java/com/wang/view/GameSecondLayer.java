 ...
 */
public class GameSecondLayer extends ModelLayer {
    //标签
    private final int tagBack = 0;
    private final int tagYun = 2;
    ...
    private void initView() {
        //添加背景
        CCSprite back = viewHelp.AddBack("gameback2.png", windowSize.width, windowSize.height);
        ...
        for(Map.Entry<Integer,CCSprite> entry: GameData.aniSelectMap.entrySet()){
            ...
            aniHight += sprite.getContentSize().height/2+30;
            aniWidth = sprite.getContentSize().width / 2 + 10;
            sprite.setScale(0.6f);
            int tag = entry.getKey();
            sprite.setPosition(sprite.getContentSize().width / 2 + 10, aniHight);
    ...
    private void YunFlow() {
        //云朵飘移
        CCSprite yun = CCSprite.sprite("litteryun.png");
        yun.setScale(0.6f);
        yun.setPosition(windowSize.width/4,windowSize.height/8);
        this.addChild(yun, tagYun, tagYun);
        ...
        CCSequence seq = CCSequence.actions(sp,sp.reverse());
            ...
            public void run() {
                try {
                    while (isRun){
                        Thread.sleep(1000);
                        time++;
                        if(time == GameData.SecondGameTime){
                            ...
                            CCDirector.sharedDirector().getActivity().runOnUiThread(new Runnable() {
                                ...
                                public void run() {
                                    ...
                                    View view = inflater.inflate(R.layout.gameover_layout, null);
                                    ...
                                    lp.x = 0; // 新位置X坐标
                                    lp.y = (int) windowSize.height / 5; // 新位置Y坐标
                                    lp.width = (int) windowSize.width; // 宽度
                                    lp.height = (int) (3*windowSize.height / 5); // 高度
    ...
    float[][] pos = {{x/3,y/3},{2*x/3,y/2},{x/2,2*y/3},{3*x/4,5*y/6},{x/2,y}};
    ...
    public void AddMon(float f){
       ...
       CCSprite di = CCSprite.sprite(String.format("di%d.png", num));
        ...
        di.setPosition(aniX,aniY);
        this.addChild(di, tagDi, tagDi);
        listDi.add(di);

        CCMoveTo to1 = CCMoveTo.action(5f, CGPoint.ccp(pos[random.nextInt(3)][0],pos[random.nextInt(3)][1]));
        CCMoveTo to2 = CCMoveTo.action(6f, CGPoint.ccp(pos[random.nextInt(3)][0],pos[random.nextInt(3)][1]));
        CCMoveTo to3 = CCMoveTo.action(6f, CGPoint.ccp(pos[random.nextInt(3)][0],pos[random.nextInt(3)][1]));
        CCMoveTo to6 = CCMoveTo.action(6f, CGPoint.ccp(pos[4][0],pos[4][1]));
        CCSequence seq = CCSequence.actions(to1, to2,to3,to6);
        di.runAction(seq);
    }
    ...
    private void AddRole() {
        ...
        role.setPosition(windowSize.width / 2, y-role.getContentSize().height/2-10);
    }
    ...
    public boolean ccTouchesEnded(MotionEvent event) {
        ...
        if(isTouch && point.x>=windowSize.width/5 && putAni!=null){
